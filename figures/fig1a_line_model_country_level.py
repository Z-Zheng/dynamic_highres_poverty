...
df = df[df['site'].isin(['mi_18', 'mz_17', 'mg', 'bf'])]
...
df.loc[df['model'] == 'Transformer_geof', 'model'] = 'Transformer w/ geo-features'

g = sns.relplot(
    df, x='frac', y='r2', hue='model', col='site',
    col_wrap=2, kind='line', height=4, aspect=1,
    col_order=['mi_18', 'mz_17', 'mg', 'bf'],
    hue_order=['CNN', 'Transformer', 'XGBoost w/ sat-features', 'XGBoost w/ geo-features', 'XGBoost w/ sat + geo-features'],
    markers=True,
    style='model',
    facet_kws=dict(sharex=False, sharey=True)
)

g._legend.set_bbox_to_anchor((0.33, 0.7))
